...
impl SType {
    fn imm(&self) -> u32 {
        ((((self.0 & 0xfe_00_00_80) as i32) >> 25) as u32) << 5
...
impl JType {
    fn imm(&self) -> u32 {
        ((((self.0 & 0x80_00_00_00) as i32) >> 31) as u32) << 20
...
fn op_store(cpu: &mut Cpu, bot: &Bot, _cycle: u8, inst: u32) -> bool {
    ...
    if target >= 0xf0_00_00_00 {
        ...
        match size {
            ...
            1 => {
                cpu.memory[target + 0] = (cpu.registers[rs2] >> 8) as u8;
                cpu.memory[target + 1] = cpu.registers[rs2] as u8;
            },
            2 => {
                cpu.memory[target + 0] = (cpu.registers[rs2] >> 24) as u8;
