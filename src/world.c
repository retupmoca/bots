...
void bots_world_tick(bots_world *w) {
  ...
  for (s = w->shots[0], i = 0; s; s = w->shots[++i]) {
    ...
    for (j = 0; j < w->num_tanks; j++) {
      if (s->x >= w->tanks[j]->x - 5 && s->x <= w->tanks[j]->x + 5 &&
          s->y >= w->tanks[j]->y - 5 && s->y <= w->tanks[j]->y + 5) {
        /* hit! */
        hit = 1;
  ...
  for (i = 0; i < w->num_tanks; i++) {
    ...
    w->tanks[i]->turret_offset =
        (w->tanks[i]->turret_offset + turret_steering) % 256;
    w->cpus[i]->ports[4] = 0;
    w->cpus[i]->ports[5] = 0;
     ...
     */
    if (w->cpus[i]->ports[0x0d] && w->cpus[i]->ports[7] &&
        !w->cpus[i]->ports[0x0b])
      scanner_steering = (scanner_steering + steering) % 256;

    w->tanks[i]->scanner_offset =
        (w->tanks[i]->scanner_offset + scanner_steering) % 256;
    w->cpus[i]->ports[8] = 0;
    w->cpus[i]->ports[9] = 0;
  }
