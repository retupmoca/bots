...
impl Cpu {
    pub fn cycle(&mut self, bot: &Bot) {
        ...
        let mut instruction  = (self.memory[self.pc as usize] as u32) << 24;
        instruction |= (self.memory[(self.pc + 1) as usize] as u32) << 16;
        instruction |= (self.memory[(self.pc + 2) as usize] as u32) << 8;
        instruction |= self.memory[(self.pc + 3) as usize] as u32;
